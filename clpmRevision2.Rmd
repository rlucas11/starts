...
In this section, I argue that the "between-person effects" that @orth_effect_2022 hope to obtain can almost never be correctly estimated using the CLPM and that their claims about the limitations of the RI-CLPM and related models reflect a misunderstanding of these models. I also briefly discuss the fact that additional models beyond the RI-CLPM exist and can address at least some of the conceptual concerns about the RI-CLPM that remain after these misconceptions are corrected. 
...
orth_testing_2021 noted that their defense of the CLPM is based on a preference for a model in which lagged associations reflect between-person causal effects. Can the cross-lagged paths in the CLPM be interpreted in this way? Unfortunately, in most cases, they cannot. This is due to the fact that a critical (and rarely discussed) assumption underlying the CLPM is likely to be violated in the types of data that psychologists and other social scientists are likely to use. Specifically, an important assumption in the CLPM is that the wave-specific disturbances (the *U*s in Figure \@ref(fig:riclpmFig)) are uncorrelated with the variables measured at prior waves. In other words, the standard CLPM requires that each variable in the model has a lag-1 autoregressive structure, where the association between any two waves is a function of the lag-1 stability and the number of lags between them. For instance, if a variable like self-esteem conformed to a lag-1 autoregressive structure and it had a one-year stability of .75, then the expected correlation for measures separated by two years would be .75 * .75 = `r .75 * .75`

With two waves of data, it is impossible to evaluate this assumption. However, when more than two waves of data exist, it is possible to examine whether correlations decline in a manner that is consistent with a lag-1 autoregressive structure. To test the plausibility of this assumption, I selected ten diverse variables that have been included in almost every wave of the long-running Household Income and Labour Dynamics in Australia (HILDA) panel study, which now spans 20 waves of assessment [@watson_hilda_2012]. I intentionally selected variables from different domains, variables those that might have different psychometric properties due to how easily observable they are (e.g., weight and income are likely to be measured with less systematic and random error than life satisfaction or social support), and variables that might change in different ways over time. 

...
* The RI-CLPM supposedly separates between-person effects from within-person effects. The benefit of doing so is that the RI-CLPM controls for stable-trait-level confounds that are not controlled in the CLPM (because a variable measured at any single wave does not perfectly capture this stable trait variance).
* The interpretation of cross-lagged effects in the RI-CLPM is: The effect of an individual's deviation from their usual level of the predictor (i.e., their mean) on future deviations from their usual level of the outcome.
* Critics of the RI-CLPM argue that sometimes we don't want to focus on this within-person effect, in which case the CLPM would be preferable. Cross-lagged paths in the CLPM are thought to reflect whether one's position relative to others on the predictor affects change in the outcome.
...
The cross-lagged panel model (CLPM) is a widely used technique for examining causal processes in longitudinal data. Hamaker et al. (2015) pointed out that the CLPM does not adequately account for stable-trait-level confounds, and they suggested the random-intercept cross-lagged panel model (RI-CLMP) as an alternative. They showed that failure to account for the random intercept can lead to incorrect conclusions about cross-lagged paths (and those effects can positively or negatively biased if the random intercept is not included). Hamaker et al. described the RI-CLPM as a multilevel model that separates between-person effects from within-person effects. 

Critics (e.g., Orth et al., 2021; Lüdtke and Robitsch, 2021) have argued that sometimes, we are actually interested in the between-person effects that a classic CLPM tests, and that the choice of model depends on one's theories about the process. Even if their arguments about the interpretation of the RI-CLPM were correct, I would disagree (we should also test plausible alternatives and then justify why our preferred model should actually be preferred). However, I also am concerned that their interpretation of the difference between the CLPM and RI-CLPM is not quite accurate. 

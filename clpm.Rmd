...
* The cross-lagged panel model (CLPM) is a commonly used method for analyzing longitudinal data. Significant cross-lagged paths are interpreted to mean that when individuals have a high standing on a predictor (relative to others), they will experience a subsequent rank-order increase in the outcome.
* The random-intercept cross-lagged model (RI-CLPM) was developed to address concerns about the confounding effects of stable-trait-level associations that are not adequately accounted for in the CLPM. 
* The RI-CLPM controls for stable-trait-level confounds that are not controlled in the CLPM (because a variable measured at any single wave does not perfectly capture this stable trait variance).
* The RI-CLPM has been described as separating between-person effects from within-person effects. 
* The standard interpretation of cross-lagged effects in the RI-CLPM is: The effect of an individual's deviation from their usual level of the predictor (i.e., their mean) on future deviations from their usual level of the outcome. 
* Critics of the RI-CLPM argue that sometimes we don't want to focus on this within-person effect, in which case the CLPM would be preferable. 
* But I think that the standard interpretation of the cross-lagged part of the RI-CLPM is incorrect. Not all between-person variance is included in the Stable Trait component. The RI-CLPM only separates *completely stable trait variance* from everything else. This means that a more precise interpretation of the cross-lagged paths should be something more like "the effect of an individual's deviation from the stable trait for the predictor on their later deviation from their stable trait for the outcome." I think (and that's the point of this document) that this is conceptually (and empirically) very different than the interpretation described above.
* If the cross-lagged, autoregressive part of the RI-CLPM model does not reflect purely within-person processes, then it negates the primary objection to using the RI-CLPM. 
* At the very least, we need some clarification of what "within-person" means in this context. 

## The Cross-Lagged Panel Model

The cross-lagged panel model (CLPM) is a widely used technique for examining causal processes in longitudinal data. With at least two waves of data, it is possible to estimate the association between Time 1 X and Time 2 Y, controlling for Time 1 Y. With some assumptions, this association can be interpreted as a causal effect of X on Y.

Hamaker et al. (2015) pointed out that the CLPM does not adequately account for stable-trait-level confounds, and they suggested the random-intercept cross-lagged panel model (RI-CLMP) as an alternative. They showed that failure to account for the random intercept can lead to incorrect conclusions about cross-lagged paths. Hamaker et al. described the RI-CLPM as a multilevel model that separates between-person effects from within-person effects. Full disclosure at the outset: I like the RI-CLPM approach a lot, and I think it should almost always (maybe always) be used in place of the CLPM when the data allow for it.^[For instance, you need at least three waves of data to estimate the RI-CLPM]

Critics (e.g., Orth et al., 2021; Lüdtke and Robitsch, 2021) have argued that sometimes, we are actually interested in the between-person effects that a classic CLPM tests, and that the choice of model should depend on one's theories about the underying process. Even if their arguments about the interpretation of the RI-CLPM were correct, I would disagree (we should also test plausible alternatives and then justify why our preferred model should actually be preferred). However, I also am concerned that their interpretation of the difference between the CLPM and RI-CLPM is not quite accurate. 

I think that two sources of confusion are (a) the ambiguous nature of autoregressive effects in terms of whether they should be described as within- or between-person effects, (b) differences in how people are using the terms "within" and "between," and (c) how the interpretation of the autoregressive (and cross-lagged) part changes depending on whether we include a random intercept. 
...
My point is that adding this stable trait variance (and modeling it) doesn't suddenly make the cross-lagged part any more "within-persons" than it was before. Orth and Lüdtke state that a regular cross-lagged panel model is not a within-persons model; it's supposed benefit is that the cross-lagged paths meaningfully link between person variance at one point in time to change in the outcome at the next time. But in my example above, the cross-lagged part gets at the same hypothetical processes in both cases. This also means that the interpretation of those cross-lagged paths *would not* be that "when individuals have higher self-esteem than usual, they will experience a subsequent increase in life satisfaction." The difference is much subtler; it would be something like "when individuals report a higher self-esteem than would be predicted from stable method variance alone, then they will experience a subsequent increase in life satisfaction." Contrary to what Orth or Lüdtke argue, this interpretation still links *individual differences* at Time 1 (now isolated from stable method variance) to change in life satisfaction at Time 2. 
...
Again, the issue is that a person's "Usual Level" is not the same thing as their "Stable Trait." These are conceptually two different things---sometimes two *very* different things. The differences depends on how much stable trait variance there is and how stable the autoregressive part is. For instance, we can use the following code to create data with quite a bit of stability, even though it has a purely autoregressive (plus cross-lags) structure. In this case, let's assume that autoregressive stability is .90, which means that after 10 waves the stability is still `r round(.9^10, digits=2)` (actually slightly higher because of the cross-lagged effects). With fewer waves, this would look even more "traity," even though there is no stable trait variance at all. People certainly have meaningful averages, even in these data.
...
The RI-CLPM again recovers the correct parameters (I won't show this, but you can check). However, what people are "typically like" (i.e., their means across all waves) is only slightly related to the actual stable trait variable (which we have access to because we generated the scores separately). The following table shows the correlations between the means for the combined Trait/AR data and the means for the two components. People's average scores are only weakly correlated with the stable trait. 
...
The reason I think that this is important is that Orth and Lüdtke's primary argument against using the RI-CLPM is that we often don't want to know whether "deviations from a person's mean at Time 1 predict their deviations from their mean in a different variable at Time 2." But I don't think that is what the RI-CLPM does, and if that interpretation is wrong, then that takes away their primary reason for avoiding modeling random intercepts. They might be able to reframe in a way that allows them to argue that they aren't interested in a what's actually left after pulling out the random intercept, but I think it would be a different argument than they currently make, and I think it would necessarily be much more convoluted. 
